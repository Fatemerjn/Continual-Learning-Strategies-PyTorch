...
def main():
    ...
    cl_strategy = GenerativeReplay(device=DEVICE, num_tasks=NUM_TASKS)
    ...
    for task_id in range(NUM_TASKS):
        ...
        for epoch in range(EPOCHS_PER_TASK):
            ...
            for images, labels in pbar:
                ...
                # Loss on replayed synthetic data
                if task_id > 0:
                ...
                loss.backward()
                optimizer.step()
                pbar.set_postfix(loss=loss.item() / (images.size(0) * (task_id + 1)))
            
            scheduler.step()
        
        # Train the generator for the task that just finished
        ...
        for i in range(task_id + 1):
            ...
            acc = evaluate(solver_model, test_loader, DEVICE, task_id=i)
